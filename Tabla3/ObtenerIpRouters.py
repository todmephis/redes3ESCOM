...
while i<len(va)-1:
	os.system("tftp "+ str(va[i][0]) +" << TFTP\n get startup-config ../conf/r"+str(i)+"\nquit\n TFTP")
	i=i+1

i=1
while i<len(va)-1:
	with open ('../conf/r'+str(i),'r') as f: #Aqui cambia la ruta a donde tengas guardados los archivos de configuración de los routers, aqui tomo como base que se llaman r'número de archivo de configuración'
...
with open("routers.txt", "w") as file:
...
f = open ('../outputs/ip_routers.txt','w') #En este archivo solo se guarda la ip de los host
f.write(lista_ips)
f.close()
