...
def polling(lista3):
    ...
    #cdprint("[DEBUG] {}".format(bot.get_me()))
    i=0
    lista1= []
    lista2= []
    contPing = 0
    ...
    while i<len(lista3):
        ...
        if response ==0:
            ...
            if response ==0:
                ...
                #bot.send_message(chat_id="8288143", text=notificacion_t)
...
def write_template(lista1,lista2):
    ...
    outF = open("inputs/message.txt", "w")
...
def send_mail(names,emails,message_template):
    # SMTP Server:
    s = smtplib.SMTP(host='mail.todmephis.cf', port=26)
    s.starttls()
...
def main():
    ...
    #print("Marca")
    #names, emails = get_contacts('inputs/mycontacts.txt') # contactos
    #message_template = read_template('../inputs/message.txt')
